...
public:
  ...
  // TODO: IMPLEMENT THIS.
  template <bool C>
  bool operator==(const SlistIterator<T, C> &other) const { return (this->m_node == other.base()); }
};
...
public:
  ...
  /*
  m_head = std::make_unique<node_type>(nullptr, *(other.begin()));
  auto it = other.begin();
  iterator temp = iterator(m_head);
  while (it)
  {
    ++it;
    temp = emplace_after(temp, *it);
  }
  */
  ...
  Slist(std::input_iterator auto b, std::input_iterator auto e)
  {
    if (b != e)
    {
      m_head = std::make_unique<node_type>(nullptr, *b);
  ...
  // TODO: IMPLEMENT THIS.
  void pop_front() { m_head = std::move(m_head->next); }
  ...
  iterator erase_after(const_iterator pos, const_iterator last)
  {
    ...
    while (!(iterator(it.base()->next) == last))
    {
      erase_after(it);
    }
    return iterator(pos.base()->next);
  ...
  void merge(Slist<T> &&other, auto compare)
  {
    if (this != &other && !other.empty() && !this->empty())
    {
      iterator it_this = this->begin();
      while (it_this != this->end() && !other.empty())
      {
        if (compare(other.m_head->value, *it_this))
        {
      ...
      it_this = this->begin();
      while (iterator(it_this.base()->next) != this->end() && !other.empty())
      {
        if (!compare(other.m_head->value, it_this.base()->next->value))
        {
          ++it_this;
        }
        else
        {
          other.m_head.swap(it_this.base()->next);
          it_this.base()->next->next.swap(other.m_head);
          ++it_this;
        }
      }
      if (!other.empty())
      {
        it_this.base()->next.swap(other.m_head);
      }
    }
    else if (this != &other && !other.empty() && this->empty())
    {
      m_head.swap(other.m_head);
    }
  }
  ...
  void sort(auto compare)
  {
    ...
    Slist<value_type> temp;
    iterator it_slow = this->begin();
    iterator it_fast = this->begin();
    while (!(it_fast == this->end()))
    {
      ++it_fast;
      if (it_fast == this->end())
      {
    ...
    if (this->size() == 2)
    {
      temp.m_head = std::move(this->m_head->next);
    }
    else
    {
      temp.m_head = std::move(it_slow.base()->next);
    }
    this->sort(compare);
    temp.sort(compare);
    this->merge(temp, compare);
  }
};
